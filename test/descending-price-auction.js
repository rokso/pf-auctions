...
describe('Descending Price Auction', async () => {
let signers, dpa, testToken, testAuction;
  ...
  beforeEach(async () => {
    ...
    // Deploy Register
    const DPA = await ethers.getContractFactory(
      'DPAMock',
      signers[0]
    );

    const TestToken = await ethers.getContractFactory('OzERC20PresetMinterPauser', signers[0]);
    testToken = await TestToken.deploy('Test', 'TST');
    await testToken.deployed();
    await testToken.mint(signers[0].address, ethers.BigNumber.from("100000000000000000000"));
    await testToken.mint(signers[1].address, ethers.BigNumber.from("100000000000000000000"));
  ...
  describe('createAuction', async () => {
    it('Should create an auction', async () => {
      await testToken.approve(dpa.address, ethers.BigNumber.from("50000000000000000000"))
    ...
    it('Should fail create an auction when tokens will not escrow', async () => {
      await expect(dpa.createAuction(testAuction)).to.be.revertedWith('ERC20: transfer amount exceeds allowance')
    })
  })

  describe('stopAuction', async () => {
    beforeEach(async () => {
      await testToken.approve(dpa.address, ethers.BigNumber.from("50000000000000000000"))
      await dpa.createAuction(testAuction)
    })

    it('Should stop an auction and return tokens', async () => {
      const id = 1;
  ...
  describe('collections', async () => {
    beforeEach(async () => {
      await testToken.approve(dpa.address, ethers.BigNumber.from("100000000000000000000"))
      testAuction.tokenAmounts = [ethers.BigNumber.from("10000000000000000000")]
    })

    it('Should create a collection', async () => {
      await expect(dpa.createCollection()).to.emit(dpa, 'CollectionCreated').withArgs(1, signers[0].address)
    })

    it('Should transfer a collection', async () => {
      await dpa.createCollection()
      const cId = 1;
      await expect(dpa.transferCollection(signers[1].address, cId)).to.emit(dpa, 'CollectionTransfer').withArgs(1, signers[0].address, signers[1].address)
    })

    it('Should create an auction within a collection', async () => {
      await dpa.createCollection();
    ...
    it('Should fail to create an auction in an unowned collection', async () => {
      await dpa.createCollection();
      testAuction.collectionId = 1;
      const dpaUserOne = dpa.connect(signers[1])
      await expect(dpaUserOne.createAuction(testAuction, {from: signers[1].address})).to.be.revertedWith('caller-not-collection-owner');
    })

    it('Should get all auctions within a collection', async () => {
      await dpa.createCollection();
      const cId = 1
  ...
  describe('auctioneers', async () => {
    beforeEach(async () => {
      await testToken.approve(dpa.address, ethers.BigNumber.from("100000000000000000000"))
      testAuction.tokenAmounts = [ethers.BigNumber.from("10000000000000000000")]
    })

    it('Should get all auctions by a single auctioneer', async () => {
      await dpa.createAuction(testAuction);
      await dpa.createAuction(testAuction);
      await dpa.createAuction(testAuction);
      await dpa.createAuction(testAuction);
  ...
  describe('bidding', async () => {
    beforeEach(async () => {
      ...
      await ttTwo.approve(dpa.address, ethers.BigNumber.from("100000000000000000000"))
      testAuction.tokenAmounts = [ethers.BigNumber.from("10000000000000000000")]
    })

    it('Should win an auction and all parties should get relevant proceeds', async () => {
      await dpa.createAuction(testAuction);
      const dpaUserOne = dpa.connect(signers[1]);
      ...
      expect(ownerBalAfter.add(bidderBalAfter)).to.equal(ethers.BigNumber.from("200000000000000000000"))
    })
  })

  describe('getCurrentPrice', async () => {
    it('Should fail to get the price if auctionId is invalid', async() =>{
      await expect(dpa.getCurrentPrice(0)).to.be.revertedWith('no-such-auction-id');
    })

    it('Should get the price given an auctionId', async() => {
      await testToken.approve(dpa.address, ethers.BigNumber.from("100000000000000000000"))
      await dpa.createAuction(testAuction);
    ...
    it('Should properly calculate the price given the ceiling, floor, and current time', async () => {
      const s = 0;
      const e = 10;
      const c = 20;
      const f = 10;
    ...
    it('Should fail to calculate if the price is not descending', async () => {
      const s = 0;
      const e = 10;
      const c = 20;
      const f = 25;
      const t = 5;
      await expect(dpa.getCurrentPriceTest(c,f,s,e,t)).to.be.revertedWith('price-not-descending');
    })

    it('Should fail to calculate if the ramp is not valid', async () => {
      const s = 20;
      const e = 10;
      const c = 20;
      const f = 25;
      const t = 5;
      await expect(dpa.getCurrentPriceTest(c,f,s,e,t)).to.be.revertedWith('invalid-ramp');
    })

    it('Should fail to calculate if the time is invalid', async () => {
      const s = 5;
      const e = 10;
      const c = 20;
      const f = 25;
      const t = 1;
      await expect(dpa.getCurrentPriceTest(c,f,s,e,t)).to.be.revertedWith('invalid-time');
    })

    it('Should return the floor if the time is past the end', async () => {
      const s = 0;
      const e = 10;
      const c = 20;
      const f = 10;
    ...
    it('Should return the ceiling if there is no ramp', async () => {
      const s = 0;
      const e = 10;
      const c = 20;
      const f = 20;
